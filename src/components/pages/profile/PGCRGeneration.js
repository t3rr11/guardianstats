...
export function generate(PGCRs, activities, currentActivity, props) {
  ...
  else { return <Loader statusText="Still loading PGCR. Please wait..." /> }
}
...
const raidPGCR = (ManifestActivities, ManifestItems, ManifestModifiers, PGCRs, activities, currentActivity, props) => {
  ...
  const activityData = activities.find(e => e.activityDetails.instanceId === activity.activityDetails.instanceId);
  ...
  if(modeType === 'pvp') {
        ...
        if(playerData.values.team.basic.value === 17.0) {
          return (
            <div key={ playerData.player.destinyUserInfo.membershipId } className={ checkPlayer(playerData.player.destinyUserInfo.displayName, modeType, team) } id={`player_${ playerData.player.destinyUserInfo.membershipId }`}>
              <div style={{ textAlign: 'left' }}><span className="pgcrPlayerName" id={ playerData.player.destinyUserInfo.membershipId } onClick={ (() => props.foundUser(playerData.player.destinyUserInfo.membershipId)) }>{ playerData.player.destinyUserInfo.displayName }</span></div>
    ...
    if(team === 'alpha') {
      ...
      return PGCRs[currentActivity].entries.map(function (playerData) {
        if(playerData.values.team.basic.value === 17.0) {
          return (
            <div key={ playerData.player.destinyUserInfo.membershipId } className={ checkPlayer(playerData.player.destinyUserInfo.displayName, modeType, team) } id={`player_${ playerData.player.destinyUserInfo.membershipId }`} onClick={ (() => props.foundUser(playerData.player.destinyUserInfo.membershipId)) }>
    ...
    if(team === 'alpha') {
      ...
      return PGCRs[currentActivity].entries.map(function (playerData) {
        if(playerData.values.team.basic.value === 17.0) {
          return (
            <div key={ playerData.player.destinyUserInfo.membershipId } className={ checkPlayer(playerData.player.destinyUserInfo.displayName, team) } id={`player_${ playerData.player.destinyUserInfo.membershipId }`} onClick={ (() => props.foundUser(playerData.player.destinyUserInfo.membershipId)) }>
              <div style={{ textAlign: 'left' }}><span className="pgcrPlayerName" id={ playerData.player.destinyUserInfo.membershipId }>{ playerData.player.destinyUserInfo.displayName }</span></div>
              <div><span>{ playerData.values.score.basic.displayValue }</span></div>
              <div><span>{ playerData.values.kills.basic.displayValue }</span></div>
              <div><span>{ playerData.values.deaths.basic.displayValue }</span></div>
    ...
    <div key={ playerData.player.destinyUserInfo.membershipId } className={ modeType === "pvp" ? "pgcrExtendedInfo pvp" : (modeType === "gambit" ? "pgcrExtendedInfo pvp" : "pgcrExtendedInfo other") }>
      <div className="pgcrExtendedInfoStats">
        <div className="innerDiv" style={{ backgroundImage: `url("https://bungie.net${ ManifestItems[playerData.player.emblemHash].secondaryIcon }"` }}>
          <div className="pgcrExtendedPlayerName" onClick={ (() => props.foundUser(playerData.player.destinyUserInfo.membershipId)) }>{ playerData.player.destinyUserInfo.displayName }</div>
        </div>
        ...
        <div className="pgcrExtendedInfoMedalsEarned">
          {
            Object.keys(playerData.extended.values).map(function(medal) {
              if(medal.includes('medal')) {
                return (
                  <div key={ uuid.v4() } className="medalContainer">
                    {
